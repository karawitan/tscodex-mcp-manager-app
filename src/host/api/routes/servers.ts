...
export function createServerRoutes(router: Router, ctx: RouteContext): void {
  ...
  router.post('/api/packages/clone', async (req: Request, res: Response) => {
    try {
      ...
      // Method 1: Use 'which' command with explicit PATH
      try {
        const enhancedPath = '/usr/bin:/usr/local/bin:/opt/homebrew/bin:/opt/local/bin:/usr/local/git/bin:' + (process.env.PATH || '');
        const { stdout: whichResult } = await execAsync('which git', {
          timeout: 5000,
          env: { ...process.env, ...proxyEnv, PATH: enhancedPath }
        });
        gitPath = whichResult.trim();
        console.log('[git-clone] Found git via enhanced which:', gitPath);
      } catch (error) {
        console.log('[git-clone] Enhanced which git failed, trying other methods');
      }
      ...
      try {
        // Prepare git command with proxy support
        let gitCommand = gitPath;
        ...
        gitCommand += ` clone "${gitUrl}" "${cloneDir}"`;
